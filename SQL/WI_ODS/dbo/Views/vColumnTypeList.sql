...
select 
	...
	--	+ case when isRequired = 1 then ' NOT NULL' else ' NULL' end
		...
		case (select count(*) from pk where pk.TABLE_NAME = OBJECT_NAME(o.object_id) and pk.TABLE_SCHEMA = SCHEMA_NAME(o.schema_id) and pk.COLUMN_NAME = c.[name]) 
		  when 1 then 'PK' else '-' end as IsPK
...
select b.SchemaName, 
b.TableName, b.ColName, b.column_id, b.TableName+'.'+b.ColName as FullName, b.ColDesc, cast(b.isRequired as char(1)) + case when b.IsPK = 'PK' then ' (PK)' else '' end as isRequired
from b