...
--CREATE PROCEDURE [zef2DP].[AKCUS_zmiana_CzyOstatniaWersjaDeklaracji]

--  AS
--    BEGIN

--	-- Part 1
--	-- Wypełnienie tabeli pomocniczej AKCUS_Pomoc
--	TRUNCATE TABLE [zef2DP].[AKCUS_Pomoc];
...
--	FROM [zef2DP].[AKCUS_Deklaracja]


--	-- Part 2
--	-- wypełnienie kolumny CzyOstatniaWersjaDeklaracji
--	UPDATE [WA_Hurtownia].[zef2DP].[AKCUS_Deklaracja] 
...
--			SELECT IdentyfikatorDokumentu FROM [WA_Hurtownia].[zef2DP].[AKCUS_Deklaracja] WHERE DataZlozeniaDeklaracjiPierwotnej IS NULL;
--			OPEN cur;
...
--			BEGIN			
--					SET @DataZlozeniaDok = 
--					(SELECT MIN([DataZlozeniaDok]) FROM [WA_Hurtownia].[zef2DP].[AKCUS_Pomoc]
--					WHERE [NumerDokumentuMain] = (
--					SELECT [NumerDokumentuMain] FROM [WA_Hurtownia].[zef2DP].[AKCUS_Pomoc] WHERE [IdentyfikatorDokumentu] = @IdentyfikatorDokumentu
--					))
