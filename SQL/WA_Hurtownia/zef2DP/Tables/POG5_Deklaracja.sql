CREATE TABLE [zef2DP].[POG5_Deklaracja] (
    ...
    CONSTRAINT [POG5_Deklaracja_PK] PRIMARY KEY CLUSTERED ([IdentyfikatorDokumentu] ASC) ON [FG_ZEF2DP]
);


GO
CREATE NONCLUSTERED INDEX [NonClusteredIndex-20160803-121650]
    ON [zef2DP].[POG5_Deklaracja]([NumerDokumentu] ASC)
    ON [FG_ZEF2DP];
...
CREATE TRIGGER [zef2DP].[POG5_DeklaracjaTrigger]
   ON  [zef2DP].[POG5_Deklaracja]
   AFTER INSERT,UPDATE
AS 
BEGIN
...
FROM [zef2DP].[POG5_Deklaracja]
WHERE NumerDokumentu like @NumerDokumentuTemp and LEN(NumerDokumentu) BETWEEN LEN(@NumerDokumentuTemp)-1 AND LEN(@NumerDokumentuTemp)+2
OPEN cur;
...
BEGIN
 ...
 UPDATE [zef2DP].[POG5_Deklaracja] SET CzyOstatniaWersjaDeklaracji = 'false' WHERE NumerDokumentu = @current_template_id;
...
UPDATE[zef2DP].[POG5_Deklaracja]
SET CzyOstatniaWersjaDeklaracji = 'true' 
WHERE NumerDokumentu = 
 (SELECT MAX(NumerDokumentu) AS NumerDokumentu FROM [zef2DP].[POG5_Deklaracja]
 where NumerDokumentu like @NumerDokumentuTemp and StatusDokumentu not like 'Anulowany%'
 AND LEN(NumerDokumentu) BETWEEN LEN(@NumerDokumentuTemp)-1 AND LEN(@NumerDokumentuTemp)+2) and StatusDokumentu not like 'Anulowany%'
