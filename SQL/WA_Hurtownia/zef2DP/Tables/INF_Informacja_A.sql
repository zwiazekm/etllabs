CREATE TABLE [zef2DP].[INF_Informacja_A] (
    ...
    CONSTRAINT [INF_Informacja_A_PK] PRIMARY KEY CLUSTERED ([IdentyfikatorDokumentu] ASC) ON [FG_ZEF2DP]
);


GO
CREATE NONCLUSTERED INDEX [NonClusteredIndex-20160803-121424]
    ON [zef2DP].[INF_Informacja_A]([NumerDokumentu] ASC)
    ON [FG_ZEF2DP];


GO
CREATE TRIGGER [zef2DP].[INFA_DeklaracjaTrigger]
   ON  zef2DP.INF_Informacja_A
   AFTER INSERT,UPDATE
AS 
BEGIN
...
BEGIN
 ...
 UPDATE [zef2DP].[INF_Informacja_A] SET CzyOstatniaWersjaDeklaracji = 'false' WHERE NumerDokumentu = @current_template_id;
...
UPDATE[zef2DP].[INF_Informacja_A]
SET CzyOstatniaWersjaDeklaracji = 'true' 
WHERE NumerDokumentu = 
 (SELECT MAX(NumerDokumentu) AS NumerDokumentu FROM [zef2DP].[INF_Informacja_A]
