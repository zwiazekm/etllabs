CREATE TABLE [zef2DP].[PKOPMS_Deklaracja] (
    ...
    CONSTRAINT [PKOPMS_Deklaracja_PK] PRIMARY KEY CLUSTERED ([IdentyfikatorDokumentu] ASC) ON [FG_ZEF2DP]
);
...
CREATE TRIGGER [zef2DP].[PKOPMS_DeklaracjaTrigger]
   ON  [zef2DP].[PKOPMS_Deklaracja]
   AFTER INSERT,UPDATE
AS 
BEGIN
...
BEGIN
 ...
 UPDATE [zef2DP].[PKOPMS_Deklaracja] SET CzyOstatniaWersjaDeklaracji = 'false' WHERE NumerDokumentu = @current_template_id;
...
UPDATE[zef2DP].[PKOPMS_Deklaracja]
SET CzyOstatniaWersjaDeklaracji = 'true' 
WHERE NumerDokumentu = 
 (SELECT MAX(NumerDokumentu) AS NumerDokumentu FROM [zef2DP].[PKOPMS_Deklaracja]
