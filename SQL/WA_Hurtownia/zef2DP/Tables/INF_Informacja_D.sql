CREATE TABLE [zef2DP].[INF_Informacja_D] (
    ...
    CONSTRAINT [INF_Informacja_D_PK] PRIMARY KEY CLUSTERED ([IdentyfikatorDokumentu] ASC) ON [FG_ZEF2DP]
);


GO
CREATE NONCLUSTERED INDEX [NonClusteredIndex-20160803-121501]
    ON [zef2DP].[INF_Informacja_D]([NumerDokumentu] ASC)
    ON [FG_ZEF2DP];


GO
CREATE TRIGGER [zef2DP].[INFD_DeklaracjaTrigger]
   ON  zef2DP.INF_Informacja_D
   AFTER INSERT,UPDATE
AS 
BEGIN
...
BEGIN
 ...
 UPDATE [zef2DP].[INF_Informacja_D] SET CzyOstatniaWersjaDeklaracji = 'false' WHERE NumerDokumentu = @current_template_id;
...
UPDATE[zef2DP].[INF_Informacja_D]
SET CzyOstatniaWersjaDeklaracji = 'true' 
WHERE NumerDokumentu = 
 (SELECT MAX(NumerDokumentu) AS NumerDokumentu FROM [zef2DP].[INF_Informacja_D]
