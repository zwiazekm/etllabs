CREATE TABLE [zef2DP].[AKCEN_Deklaracja] (
    ...
    CONSTRAINT [AKCEN_Deklaracja_PK] PRIMARY KEY CLUSTERED ([IdentyfikatorDokumentu] ASC) ON [FG_ZEF2DP]
);


GO
CREATE NONCLUSTERED INDEX [NonClusteredIndex-20160803-121136]
    ON [zef2DP].[AKCEN_Deklaracja]([NumerDokumentu] ASC)
    ON [FG_ZEF2DP];


GO
CREATE TRIGGER [zef2DP].[AKCEN_DeklaracjaTrigger]
   ON  zef2DP.AKCEN_Deklaracja
   AFTER INSERT,UPDATE
AS 
BEGIN
...
BEGIN
 ...
 UPDATE [zef2DP].[AKCEN_Deklaracja] SET CzyOstatniaWersjaDeklaracji = 'false' WHERE NumerDokumentu = @current_template_id;
...
UPDATE[zef2DP].[AKCEN_Deklaracja]
SET CzyOstatniaWersjaDeklaracji = 'true' 
WHERE NumerDokumentu = 
 (SELECT MAX(NumerDokumentu) AS NumerDokumentu FROM [zef2DP].[AKCEN_Deklaracja]
