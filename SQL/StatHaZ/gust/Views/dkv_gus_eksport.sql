CREATE VIEW gust.dkv_gus_eksport as 
select left(case when isnumeric(ilosc) = 1 
       ...
       left(convert(varchar(3),kraj_przezn),3)                                           kraj_przezn, 
       left(case when isnumeric(masa_net) = 1 
            then convert(varchar(15),convert(bigint, masa_net)) 
...
--from GUS_XLS...Eksport$
from gust.dkt_gus_eksport_bufor