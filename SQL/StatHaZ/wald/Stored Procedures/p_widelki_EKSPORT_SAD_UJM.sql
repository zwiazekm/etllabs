CREATE                       PROCEDURE 	[wald].p_widelki_EKSPORT_SAD_UJM
...
BEGIN
	...
	SET @rowki_odrzut=(SELECT @@ROWCOUNT)
	INSERT INTO [wald].widelki_EKSPORT (kod_towarowy, znak, kraj_przezn, [min], [max], srednia, rowki_wejscie, rowki_odrzut) 
	SELECT @kod_cn, 'ujm_sad', @kraj_przezn, MIN(wart_cechy), MAX(wart_cechy), AVG(wart_cechy), @rowki_wejscie, @rowki_odrzut FROM wald.posrednia_k
	END
	IF @rowki_wejscie IN ('1','2')
	BEGIN
	SET @rowki_odrzut=0
