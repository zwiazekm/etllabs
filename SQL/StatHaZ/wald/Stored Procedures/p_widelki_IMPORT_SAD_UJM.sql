...
CREATE                 PROCEDURE 	[wald].p_widelki_IMPORT_SAD_UJM
...
BEGIN
	...
	SELECT @kod_cn, 'ujm_sad', 'ZZ', MIN(wart_cechy), MAX(wart_cechy), AVG(wart_cechy), @rowki_wejscie, @rowki_odrzut FROM wald.posrednia 
	END
...
BEGIN
	...
	SELECT	@kod_cn AS kod_towarowy, @kraj_poch as kraj_poch, im_wartosc_stat_zl/im_ilosc AS wart_cechy
		...
		WHERE	im_ilosc!=0 AND
				...
				@kod_cn=im_pcn and @kraj_poch=im_kraj_pochodzenia
	...
	SET @rowki_odrzut=(SELECT @@ROWCOUNT)
	INSERT INTO [wald].widelki_IMPORT (kod_towarowy, znak, kraj_poch, [min], [max], srednia, rowki_wejscie, rowki_odrzut) 
	SELECT @kod_cn, 'ujm_sad', @kraj_poch, MIN(wart_cechy), MAX(wart_cechy), AVG(wart_cechy), @rowki_wejscie, @rowki_odrzut FROM wald.posrednia_k
	END
	IF @rowki_wejscie IN ('1','2')
	BEGIN
	SET @rowki_odrzut=0
