...
SET @sql='INSERT INTO wald.taryfa_cn8 (kod_cn) SELECT  im_pcn FROM [wald].widelki_zbior_roczny_import
			 WHERE im_znacznik_kategorii_progowej NOT IN (''0'')
			 GROUP BY im_pcn ORDER BY im_pcn'
exec sys.sp_executesql @sql

SET @sql='INSERT INTO wald.cn_kraj_poch (kod_cn, kraj_poch) SELECT  im_pcn, im_kraj_pochodzenia FROM [wald].widelki_zbior_roczny_import
			 WHERE im_znacznik_kategorii_progowej NOT IN (''0'')
			 GROUP BY im_pcn, im_kraj_pochodzenia 
			 ORDER BY im_pcn, im_kraj_pochodzenia'
exec sys.sp_executesql @sql
...
BEGIN
	SET @kod_cn=(SELECT kod_cn FROM wald.taryfa_cn8 WHERE [id]=@start)
		...
		SELECT @kod_cn AS kod_towarowy, im_wartosc_stat_zl/im_masa_netto AS wart_cechy
		FROM	[wald].widelki_zbior_roczny_import
		WHERE	im_masa_netto!=0 AND 
				im_pcn = @kod_cn AND
...
BEGIN
	...
	SELECT	@kod_cn AS kod_towarowy, @kraj_poch as kraj_poch, im_wartosc_stat_zl/im_masa_netto AS wart_cechy
		FROM	[wald].widelki_zbior_roczny_import
		WHERE	im_masa_netto!=0 AND
	...
	INSERT INTO [wald].widelki_IMPORT (kod_towarowy, znak, kraj_poch, [min], [max], srednia, rowki_wejscie, rowki_odrzut) 
	SELECT @kod_cn, 'masa_intr', @kraj_poch, MIN(wart_cechy), MAX(wart_cechy), AVG(wart_cechy), @rowki_wejscie, @rowki_odrzut FROM wald.posrednia_k
	END
	IF @rowki_wejscie IN ('1','2')
	BEGIN
	SET @rowki_odrzut=0
